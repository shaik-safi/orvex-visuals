    ...
    <section className="pt-32 pb-10 md:pt-40 md:pb-14 bg-gradient-to-b from-slate-50 to-white dark:from-slate-950 dark:to-slate-900 relative overflow-hidden">
      ...
      <div ref={ref} className={`max-w-4xl mx-auto px-4 text-center transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        ...
        <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-slate-900 dark:text-white leading-[0.95] mb-6">
          Tips, Guides &{" "}
          <span className="bg-gradient-to-r from-amber-500 to-orange-500 bg-clip-text text-transparent">Inspiration</span>
        </h1>
        <p className="text-lg md:text-xl text-slate-500 dark:text-slate-400 max-w-2xl mx-auto">
          Photography tips, location guides, pricing breakdowns, and expert advice to help you plan the perfect shoot.
        </p>
      </div>
    </section>
  )
  ...
  return (
    <section ref={ref} className={`py-8 bg-white dark:bg-slate-950 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link href={`/blog/${post.slug}`} className="group grid md:grid-cols-2 gap-6 bg-slate-50 dark:bg-slate-900 rounded-3xl overflow-hidden border border-slate-100 dark:border-slate-800 hover:border-amber-300 dark:hover:border-amber-500/40 transition-all duration-500 hover:shadow-xl">
    ...
    <section className="py-10 md:py-14 bg-white dark:bg-slate-950">
      <div ref={ref} className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
          <strong className="text-slate-900 dark:text-white">{filtered.length}</strong> articles
        </p>
            ...
            >
              ...
              <div className="p-5">
                <div className="flex items-center gap-2 mb-3">
                  <span className="bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 px-2.5 py-0.5 rounded-md text-xs font-medium capitalize">

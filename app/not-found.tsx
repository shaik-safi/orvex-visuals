    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white dark:from-slate-950 dark:to-slate-900 flex items-center justify-center px-4">
      <div className="text-center max-w-lg">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-lg mb-8 leading-relaxed">
          Looks like this page wandered off frame. Let&apos;s get you back to capturing moments.
        </p>
        ...
        <div className="mt-12 pt-8 border-t border-slate-200 dark:border-slate-800">
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {[["Services", "/services"], ["Pricing", "/pricing"], ["Gallery", "/gallery"], ["Blog", "/blog"], ["Contact", "/contact"]].map(([label, href]) => (
              <Link key={label} href={href} className="px-3 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-600 dark:text-slate-300 hover:border-amber-300 dark:hover:border-amber-500/40 hover:text-amber-600 dark:hover:text-amber-400 transition-all">
                {label}
              </Link>
            ))}
          </div>

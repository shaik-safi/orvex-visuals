...
import { openPopupWindow } from "@/lib/popup-handoff"

type ContactMessages = ReturnType<typeof getPageMessages>["contactPage"]
type ContactHandoffStatus = "idle" | "opened" | "blocked"

function ContactHero({ messages }: { messages: ContactMessages }) {
  const { ref, isVisible } = useScrollReveal<HTMLDivElement>()
  return (
    <section className="pt-28 pb-10 sm:pt-32 md:pt-40 md:pb-14 bg-gradient-to-b from-slate-50 to-white dark:from-slate-950 dark:to-slate-900 relative overflow-hidden">
      <div className="absolute top-12 left-1/2 h-[min(72vw,31.25rem)] w-[min(72vw,31.25rem)] -translate-x-1/2 bg-amber-500/5 rounded-full blur-3xl pointer-events-none sm:left-auto sm:right-1/3 sm:translate-x-0" />
      <div ref={ref} className={`max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center transition-all duration-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
        ...
        <h1 className="text-4xl sm:text-5xl md:text-6xl xl:text-7xl font-bold text-slate-900 dark:text-white leading-[0.95] mb-6">
...
function ContactSection({ messages }: { messages: ContactMessages }) {
  ...
  const [handoffStatus, setHandoffStatus] = useState<ContactHandoffStatus>("idle")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setHandoffStatus(openPopupWindow(nextWhatsAppUrl) ? "opened" : "blocked")
      } else {
  ...
  return (
    <section ref={ref} className={`py-12 md:py-16 bg-white dark:bg-slate-950 transition-all duration-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
        ...
        <div className="grid lg:grid-cols-5 gap-10">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
              <a href={getWhatsAppLink()} target="_blank" rel="noopener noreferrer" className="group flex items-start gap-4 bg-green-50 dark:bg-green-500/5 rounded-2xl p-4 border border-green-100 dark:border-green-500/10 hover:border-green-300 dark:hover:border-green-500/30 transition-colors duration-300">
                <div className="w-10 h-10 bg-green-100 dark:bg-green-500/10 rounded-xl flex items-center justify-center flex-shrink-0">
              ...
              <a href={`tel:${PHONE_NUMBER}`} className="group flex items-start gap-4 bg-slate-50 dark:bg-slate-900 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 hover:border-amber-300 dark:hover:border-amber-500/30 transition-colors duration-300">
              ...
              <a href={`mailto:${EMAIL}`} className="group flex items-start gap-4 bg-slate-50 dark:bg-slate-900 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 hover:border-amber-300 dark:hover:border-amber-500/30 transition-colors duration-300">
          ...
          <div className="lg:col-span-3">
            <div className="bg-slate-50 dark:bg-slate-900 rounded-3xl p-6 md:p-8 border border-slate-100 dark:border-slate-800">
              ...
              {submitted ? (
                (() => {
                  const successDescription = submissionResult === "saved"
                    ? messages.form.savedDescription
                    : handoffStatus === "blocked"
                      ? messages.form.waBlockedDescription
                      : messages.form.waDescription

                  const whatsappLabel = submissionResult === "saved"
                    ? messages.form.continueWhatsapp
                    : handoffStatus === "blocked"
                      ? messages.form.openWhatsappNow
                      : messages.form.openWhatsappAgain
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full inline-flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white px-6 py-3.5 rounded-xl font-semibold text-sm transition-colors duration-300 disabled:opacity-60"
                  >
...
function MapSection() {
  const { ref, isVisible } = useScrollReveal()
  return (
    <section ref={ref} className={`py-12 bg-slate-50 dark:bg-slate-900 transition-all duration-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">

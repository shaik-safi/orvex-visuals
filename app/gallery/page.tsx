...
function Lightbox({ images, currentIndex, onClose, onPrev, onNext, messages }: {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-sm flex items-center justify-center" onClick={onClose}>
      ...
      <div className="max-w-[90vw] max-h-[85svh] relative" onClick={(e) => e.stopPropagation()}>
        <Image
          src={current.src}
          alt={current.alt}
          width={current.width}
          height={current.height}
          className="max-w-full max-h-[85svh] object-contain rounded-lg shadow-2xl"
          sizes="90vw"
          quality={90}
          priority
        />
...
function GalleryHero({ messages }: { messages: GalleryMessages }) {
  ...
  return (
    <section className="pt-28 pb-10 sm:pt-32 md:pt-40 md:pb-14 bg-gradient-to-b from-slate-50 to-white dark:from-slate-950 dark:to-slate-900 relative overflow-hidden">
      <div className="absolute top-12 left-1/2 h-[min(72vw,31.25rem)] w-[min(72vw,31.25rem)] -translate-x-1/2 bg-amber-500/5 rounded-full blur-3xl pointer-events-none sm:left-auto sm:right-1/4 sm:translate-x-0" />
      <div ref={ref} className={`max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>

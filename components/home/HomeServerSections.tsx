import { Clock, Shield, Receipt, Smartphone, ArrowRight, Users, Award } from "lucide-react"
...
const TRUST_BAR_ICONS = [Users, Receipt, Clock, Award] as const
    ...
    <div className="py-6 bg-slate-900 dark:bg-slate-800 border-y border-white/5">
      <div className="max-w-7xl mx-auto px-4 flex flex-wrap justify-center items-center gap-6 md:gap-12 text-sm">
        {messages.trustBar.map((item, index) => {
          const Icon = TRUST_BAR_ICONS[index] ?? Users
          return (
            <div key={index} className="flex items-center gap-2 text-white/70">
              <Icon size={16} className="text-amber-400" />
              <span><strong className="text-white">{item.strong}</strong> {item.label}</span>
            </div>
          )
        })}
      </div>
